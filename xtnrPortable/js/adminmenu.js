...
//set action to trigger all menus, can be 'over' or 'click'
var mouseaction = "over";
...
switch (nodes[i].className) { 
	...
	box2.style.top = (box1.offsetTop + this.position.levelX.top) + "px"; box2.style.left = (box1.offsetLeft + box1.offsetWidth + this.position.levelX.left) + "px";}
break; case "arrow":
...
function buttonClick() { 
	...
	if (self.visible.length) { 
		...
		var el = new Element(this.id); if (el.hasChilds()) {
			self.show(this.id);
		}
	}
}
